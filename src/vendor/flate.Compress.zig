...
const token = @import("flate.token.zig");
...
pub fn syncFlush(c: *Compress) Writer.Error!void {
    try rebaseInner(&c.writer, 0, c.writer.buffer.len - flate.history_len, true, false);
    // Emit a non-final empty stored block immediately after the current block.
    // Do NOT byte-align first: the stored block header must pack into the
    // remaining bits of the current byte, matching zlib's Z_SYNC_FLUSH behavior.
    try c.bit_writer.write(BlockHeader.int(.{ .kind = .stored, .final = false }), 3);
    c.bit_writer.byteAlign();
    try c.bit_writer.output.writeInt(u16, 0x0000, .little);
    try c.bit_writer.output.writeInt(u16, 0xffff, .little);
}
